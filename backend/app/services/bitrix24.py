...
class Bitrix24Service:
    ...
    async def get_tasks_stats(self) -> dict:
        """
        Получает статистику по задачам CRM.

        ЧТО: Запрашивает список задач из Bitrix24 и подсчитывает общее количество,
        завершённые и просроченные.

        ЗАЧЕМ: Статистика задач показывает дисциплину выполнения:
          - total: общий объём задач в системе.
          - completed: сколько выполнено (status == "5" в Bitrix24).
          - overdue: сколько задач просрочено (не завершены, но имеют deadline).
        Большое количество просроченных задач --- сигнал о проблемах
        с планированием или загрузкой персонала.

        КАК работает:
          1. GET-запрос к {webhook_url}/tasks.task.list с выборкой полей
             ID, STATUS и DEADLINE.
          2. Подсчёт общего количества задач.
          3. Подсчёт завершённых: status == "5" (код "Завершена" в Bitrix24).
          4. Подсчёт просроченных: status != "5" и deadline задан (упрощённая
             логика --- не сравнивает с текущей датой, а считает все незавершённые
             задачи с дедлайном потенциально просроченными).
          5. При ошибке возвращает нулевые значения.

        Ограничение: возвращает данные только по первой странице (до 50 задач).

        Возвращает:
            dict: Словарь с ключами:
                - "total" (int): Общее количество задач.
                - "completed" (int): Количество завершённых задач.
                - "overdue" (int): Количество просроченных задач.
        """
        try:
            async with httpx.AsyncClient() as client:
                response = await client.get(
